...
if ~isempty(resp)
    ...
    fprintf('Timestamp: %d\n', resp.timestamp);
    fprintf('Terminado: %d\n', resp.terminated);
    fprintf('Observación: [%.1f, %.1f]\n', resp.observation(1), resp.observation(2));
else
    error('ERROR: No se pudo comunicar con Arduino');
end

%% Parámetros del controlador PID
K_P = 0.3;      % Ganancia proporcional
K_I = 0.001;   % Ganancia integral
...
% Configurar modo de paso y establecer home
...
fprintf('Ejecutando control PID...\n');
...
for i = 1:NUM_STEPS
    ...
    if mod(i, 100) == 0
        fprintf('Paso %d: Encoder=%.1f°, Error=%.1f°, Salida=%.1f°\n', ...
                i, enc_deg, error, move_deg);
    end
...
sgtitle('Resultados del Control PID del Péndulo Invertido');
