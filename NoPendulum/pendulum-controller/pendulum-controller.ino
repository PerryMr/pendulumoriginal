...
* @details
* CAMBIOS APLICADOS:
* 1. Reemplazada apply_control_smooth() con apply_control_direct()
* 2. Eliminadas variables de acumulación complejas
* 3. Aplicación directa del control como Shawn Hymel
* 4. Mantiene sample time fijo de 20ms
*/
...
void apply_control_direct(float control_output) {
  // Aplicación directa del control como lo hace Shawn Hymel
  // Sin acumulación ni factores de suavizado complejos
  
  // Solo aplicar si el stepper está libre (como hace Shawn)
  if (stepper->get_device_state() == INACTIVE) {
    
    // CORRECCIÓN DEL SIGNO: Invertir la dirección del control
    float corrected_control = -control_output;
    
    // Aplicar directamente el control calculado (sin acumulación)
    move_stepper_by(corrected_control);
    
    // Debug periódico simplificado
