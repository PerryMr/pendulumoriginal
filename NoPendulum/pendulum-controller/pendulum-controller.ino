...
// Tiempo de muestreo 
const float SAMPLE_TIME = 0.02; // 2ms sampling time
...
const unsigned long CONTROL_INTERVAL = 20; // 4ms = 250Hz
...
*/
// Implementar el PID [INTEGRAL BACKWARDS]
...
void run_control_loop() {
  ...
  if (system_state == STATE_CONTROL && is_in_control_maintenance_range()) {
    ...
    if (abs(phi_error) > 0.001745 && stepper->get_device_state() == INACTIVE) {
...
void loop() {
  ...
  if (rx_code == ControlComms::OK) {
    ...
    observation[4] = current_controller*rad_deg; // tipo de controlador
    observation[5] = integral_error*rad_deg; // Error integral
