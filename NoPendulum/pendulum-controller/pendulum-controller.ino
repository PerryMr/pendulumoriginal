...
// Tiempo de muestreo 
const float SAMPLE_TIME = 0.01; // 5ms sampling time
...
void update_states() {
  ...
  float dphi_diff = normalize_angle(phi - phi_prev);
  float dtheta_diff = normalize_angle(theta - theta_prev);
  
  dphi = dphi_diff / dt;
  dtheta = dtheta_diff / dt;
...
const float DEADBAND = 0.05;  // ~1.7° (reducido significativamente)

// FILTRO DERIVATIVO - Parámetro crítico
const float DERIVATIVE_FILTER_COEFF = 0.;  // Filtro más agresivo (0.1 = 90% histórico, 10% nuevo)
...
float compute_pid_control() {
  ...
  float derivative = (error_prev_filtered - pid_error_prev) / SAMPLE_TIME;
  
  // Filtrar la derivada también
  derivative_filtered = DERIVATIVE_FILTER_COEFF * derivative + 
...
void run_control_loop() {
  // Leer sensores cada 5ms
  if (millis() - control_loop_timer < 5) {
    return;
  }
  control_loop_timer = millis();

  update_states();  // Actualizar siempre
  manage_control_state_transitions();

  // Enviar comandos al motor solo cada 10ms
  static unsigned long last_motor_command = 0;
  ...
  if (system_state == STATE_CONTROL && is_in_control_maintenance_range()) {
    ...
    //  Solo aplicar control cada 2 iteraciones (10ms efectivo)
